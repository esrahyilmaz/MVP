...
import static helpers.ReportHelper.customLogSuccess;

public class FindClosestMatchingElement {
    ...
    public static void findClosestMatch(EventFiringWebDriver eventFiringWebDriver, String _findByContext, String storedClassName,
        ...
        if (closestElement != null) {
            ...
            log.error(">> Target element not found.  Unable to provide suggestion to fix.");
            ReportHelper.customLogError("Target element not found.  Unable to provide suggestion to fix.");
        }
    ...
    public static void printLocators(WebElement closestElement) {
        customLogSuccess("Some WebElement Locators to try 'By.xpath' :");
        if (closestElement.getAttribute("id") != null && !closestElement.getAttribute("id").isBlank()) {
            altXpathId = "//*[@id='" + closestElement.getAttribute("id") + "']";
            customLogSuccess(altXpathId);
        }
        if (closestElement.getAttribute("name") != null && !closestElement.getAttribute("name").isBlank()) {
            altXpathName = "//*[@name='" + closestElement.getAttribute("name") + "']";
            customLogSuccess(altXpathName);
        }
        if (closestElement.getAttribute("className") != null && !closestElement.getAttribute("className").isBlank()) {
            altXpathClass = "//*[@class='" + closestElement.getAttribute("className") + "']";
            customLogSuccess(altXpathClass);
        }
        if (closestElement.getAttribute("textContent") != null && !closestElement.getAttribute("textContent").isBlank()) {
            altXpathText = "//*[text()='" + closestElement.getAttribute("textContent") + "']";
            customLogSuccess(altXpathText);
        }
    }
    ...
    public static String getAltXpathTextContent() {
        return altXpathText;
    }
    ...
    public static void printElementDetails(WebElement closestElement, String findByContext, double closestScore) {
        ...
        System.out.println("}");
        customLogSuccess("Found Best Match: Match Score is " + closestScore);
        customLogInfo("Tag Name : " + closestElement.getAttribute("tagName"));
        customLogInfo("id : " + closestElement.getAttribute("id"));
    }
}